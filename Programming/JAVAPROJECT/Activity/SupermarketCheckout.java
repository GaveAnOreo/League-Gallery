...
public class SupermarketCheckout {
    ...
    private static final Map<String, String[]> productCatalog = new HashMap<>(); // prodcode -> {prodname, unitprice_string}
    ...
    public static void main(String[] args) {
        ...
        try (Scanner localScanner = new Scanner(System.in)) {
            ...
            String username = localScanner.nextLine();
            System.out.print("Enter Password: ");
            String password = localScanner.nextLine();
            if (checkLogin(username, password)) {
                ...
                String signupChoice = localScanner.nextLine();
    ...
    private static void runPOS() {
        ...
        do {
            ...
            if (productCatalog.containsKey(productCode)) {
                ...
                try {
                    ...
                    // This should ideally not happen if loadProducts validated correctly, but as a safeguard:
        ...
        receiptBuilder.append(String.format("%35s\n", storeName.toUpperCase())); // Centered-ish
        receiptBuilder.append(String.format("%37s\n", "OFFICIAL RECEIPT"));
        receiptBuilder.append("Date: ").append(dateFormat.format(transactionDate)).append("\n");
        receiptBuilder.append("--------------------------------------------------------\n");
        receiptBuilder.append(String.format("%-20s %10s %8s %15s\n", "Product Name", "Unit Price", "Qty", "Cost"));
        receiptBuilder.append("--------------------------------------------------------\n");


        for (int i = 0; i < productNamesList.size(); i++) {
            receiptBuilder.append(String.format("%-20.20s %10s %8d %15s\n", // Truncate product name if too long
        ...
        receiptBuilder.append("--------------------------------------------------------\n");
        receiptBuilder.append(String.format("%-30s %23s\n", "Total Items:", totalQuantity));
        receiptBuilder.append(String.format("%-30s %23s\n", "Subtotal (VAT-Exclusive):", df.format(subTotalBeforeVat)));
        receiptBuilder.append(String.format("%-30s %23s\n", "VAT (12%):", df.format(vatAmount)));
        receiptBuilder.append(String.format("%-30s %23s\n", "Total (VAT-Inclusive):", df.format(totalBeforeDiscount)));
       
        if (discountPercent > 0) {
            receiptBuilder.append(String.format("%-30s %23s\n", "Discount Type:", discountName));
            receiptBuilder.append(String.format("%-30s (%s%%): %20s\n", "Discount Amount", df.format(discountPercent*100) , "-" + df.format(discountAmount)));
        }
        receiptBuilder.append("--------------------------------------------------------\n");
        receiptBuilder.append(String.format("%-30s %23s\n", "TOTAL AMOUNT DUE:", df.format(amountDue)));
        receiptBuilder.append("--------------------------------------------------------\n");
        receiptBuilder.append(String.format("%-30s %23s\n", "Amount Paid:", df.format(payment)));
        receiptBuilder.append(String.format("%-30s %23s\n", "Change:", df.format(change)));
        receiptBuilder.append("========================================================\n");
        receiptBuilder.append(String.format("%38s\n", "Thank you for shopping!"));
        receiptBuilder.append("========================================================\n\n");
