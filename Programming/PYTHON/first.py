...
class MemoryAllocatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MFP - FIRST FIT - NPP")
    ...
    def create_widgets(self):
        ...
        input_frame = ttk.LabelFrame(main_frame, text="Input Parameters (values separated by spaces or commas)", padding=15)
        ...
        metrics_frame = ttk.LabelFrame(results_frame, text="IF, EF and %MMU", padding=15)
    ...
    def create_input_rows(self, parent):
        entries = []
        fields = [
            ("OS Memory (KB):", "optional"),
            ("Partition Sizes (KB):", "required"),
            ("Memory Sizes (KB):", "required"),
            ("Process Priorities:", "required"),
            ("Burst Time:", "required"),
            ("Time:", "required")
        ]
    ...
    def allocate_memory(self):
        try:
            ...
            # Parse inputs with comma/space handling
            os_memory_str = self.os_entry.get().strip().replace(',', '')
            os_memory = int(os_memory_str) if os_memory_str else 0

            def parse_input(entry):
                return [int(x) for x in entry.replace(',', ' ').split()]

            partitions = parse_input(self.partition_entry.get())
            process_sizes = parse_input(self.process_entry.get())
            priorities = parse_input(self.priority_entry.get())
            burst_times = parse_input(self.burst_entry.get())

            time_str = self.time_entry.get().strip().replace(',', '')
            ...
            if len(process_sizes) != len(priorities) or len(process_sizes) != len(burst_times):
                messagebox.showerror("Error", 
                    "All process-related fields must have equal entries\n"
                    "(values separated by spaces or commas)")
                return

            memory = [MemoryPartition(size) for size in partitions]
            processes = [Process(i+1, process_sizes[i], priorities[i], burst_times[i]) 
            ...
            messagebox.showerror("Error", 
                ...
                "2. Separated by spaces or commas\n"
                "3. Equal entries for process-related fields")
