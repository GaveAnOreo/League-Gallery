(() => {
  ...
  const renderChampions = () => {
    ...
    for (let i = 0; i < pageItems.length; i += 1) {
      html += createCard(pageItems[i]);
    }
    elements.championList.innerHTML = html;
    renderPagination(totalPages);
  };

  const renderPlaceholders = (count = CHAMPIONS_PER_PAGE) => {
    let html = '';
    for (let i = 0; i < count; i += 1) {
      const team = Math.random() > 0.5 ? 'blue' : 'red';
      html += `
        <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
          <div class="loading-slot placeholder" data-team="${team}">
            <article class="slot-card" aria-hidden="true">
              <div class="slot-art">
              ...
              <div class="slot-overlay">
                <div class="slot-body">
                  ...
                  <h3 class="slot-name">&nbsp;</h3>
                </div>
                <div class="skeleton-btn slot-indicator">&nbsp;</div>
              </div>
            </article>
          </div>
        </div>
